...
//PRODUCTO TITULO
    ...
    h1.innerText = "Catalogo";
    let container = document.querySelector(".container");

    //ARREGLO DE PRODUCTOS & MAPEOs
    const dynamicCard = (cars) => {
      if (cars.length === 0) {
        container.innerHTML = `<p class="alert">No se encontró ningun producto. error 404.</p>`;
        return;
      }
    const cards = cars.map((productos) => 
    ...
    //BUSCADOR
      ...
      function removeBtn (){
        ...
        btnCleanAppear(); 
            ...
            // Función para mostrar o esconder btnClean.
        function btnCleanAppear(){  
  if (input.value !== '') {
      btnClean.style.display = 'inline';  // Mostrar el botón
  } else {
      btnClean.style.display = 'none';  // Esconder el botón
  }
}
// Escuchar el evento 'input' para ejecutar la función cuando cambia el mismo.
